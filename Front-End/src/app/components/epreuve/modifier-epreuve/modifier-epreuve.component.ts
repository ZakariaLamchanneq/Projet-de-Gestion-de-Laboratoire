...
export class ModifierEpreuveComponent implements OnInit {
  ...
  populateForm(): void {
    if (this.epreuve) {
      this.epreuveForm.patchValue(this.epreuve);
    }
  }
  ...
  onSubmit(): void {
    if (this.epreuveForm.valid) {
      this.loading = true;
      this.epreuveService.updateEpreuve(this.epreuve.id, this.epreuveForm.value).subscribe({
        next: () => {
          this.loading = false;
          this.message.success('Epreuve updated successfully!');
          this.modalRef.close('success');
        },
        error: () => {
          this.loading = false;
          this.message.error('Failed to update epreuve.');
        },
      });
    }
  }
